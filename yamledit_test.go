...
func TestDeleteKey_Nested_Verification(t *testing.T) {
	in := []byte(`deploy:
  envs:
...
`)
	...
	s := string(output)
	assert.NotContains(t, s, "REMOVE_THIS", "Key should have been removed")
	assert.Contains(t, s, "KEEP_THIS", "Sibling key should be preserved")
}
...
`)
	...
	err = ApplyJSONPatchAtPathBytes(doc, patchJSON1, []string{"spec"})
	require.NoError(t, err)

	result1, err := Marshal(doc)
	require.NoError(t, err)

	// Fix: expect single quotes which yamledit uses for safe insertions
	assert.Contains(t, string(result1), "newKey: 'newValue'", "Simple add failed")
	...
	err = ApplyJSONPatchAtPathBytes(doc2, patchJSON2, []string{"spec"})
	require.NoError(t, err)
	...
	s2 := string(result2)
	assert.Contains(t, s2, "argoCd:", "Nested object root missing")
	assert.Contains(t, s2, "additionalAdminGroups:", "Nested key missing")
	assert.Contains(t, s2, "- group1", "Array item missing")
}
...
func TestNestedAdditionInSequence(t *testing.T) {
	yamlInput := `
...
`
	...
	patches := []map[string]interface{}{
		{
			"op":    "add",
			"path":  "/pipelineConfig/pipelineProcess/0/plugin/destination",
			"value": map[string]interface{}{"host": "a.b.c"},
		},
	}
	...
	// Verify the host is inside the plugin, not at the root
	outStr := string(output)
	assert.Contains(t, outStr, "        destination:")
	assert.Contains(t, outStr, "          host: a.b.c")
	assert.NotContains(t, outStr, "\ndestination:") // Should not be at root
}

func TestYamleditAddOperationDeepNested(t *testing.T) {
	yamlContent := `pipelineName: test-pipeline
pipelineConfig:
  pipelineProcess:
    - processName: data_stream
      plugin:
        name: flinkstream
    - processName: sink_to_es
      plugin:
        name: flinksink
        properties:
          type: ELASTICSEARCH
...
`
	...
	doc, err := Parse([]byte(yamlContent))
	if err != nil {
		t.Fatalf("Parse error: %v", err)
	}

	// Add host to nested destination (index 1 in pipelineProcess array)
	patch := `[{"op":"add","path":"/pipelineConfig/pipelineProcess/1/plugin/destination/host","value":"a.b.c"}]`

	err = ApplyJSONPatchBytes(doc, []byte(patch))
	if err != nil {
		t.Fatalf("ApplyJSONPatchBytes error: %v", err)
	}
	...
	if !strings.Contains(string(result), "host:") {
		t.Errorf("Expected 'host:' to be added to output, but it wasn't.\nOutput:\n%s", string(result))
	}
}

func TestYamleditReplaceOperationDeepNested(t *testing.T) {
	yamlContent := `pipelineName: test-pipeline
pipelineConfig:
  pipelineProcess:
    - processName: data_stream
...
`
	...
	// Replace existing host in nested destination
	patch := `[{"op":"replace","path":"/pipelineConfig/pipelineProcess/1/plugin/destination/host","value":"updated.host.com"}]`
	...
	assert.Contains(t, string(result), "host: updated.host.com")
}

// TestYamleditAddDeepNestedAlone tests adding a new key with ONLY an add patch
func TestYamleditAddDeepNestedAlone(t *testing.T) {
	yamlContent := `pipelineName: test-pipeline
pipelineConfig:
  pipelineProcess:
    - processName: data_stream
...
`
	...
	if !strings.Contains(string(result), "host:") {
		...
		t.Logf("Output:\n%s", string(result))
		t.Errorf("BUG: 'host:' was NOT added - yamledit silently dropped the 'add' operation!")
	}
}

// TestYamleditAddDeepNestedWithReplace tests add + replace patches together
func TestYamleditAddDeepNestedWithReplace(t *testing.T) {
	yamlContent := `pipelineName: test-pipeline
pipelineConfig:
  pipelineProcess:
    - processName: data_stream
...
`
	...
	patch := `[
       {"op":"replace","path":"/pipelineName","value":"new-pipeline"},
       {"op":"add","path":"/pipelineConfig/pipelineProcess/1/plugin/destination/host","value":"a.b.c"}
    ]`
	...
	if !strings.Contains(string(result), "host:") {
		t.Logf("Patch: %s", patch)
		t.Logf("Output:\n%s", string(result))
		t.Errorf("BUG: 'host:' was NOT added!")
	}
}

// TestYamleditAddRealPipeline tests the exact YAML structure from our pipeline files
func TestYamleditAddRealPipeline(t *testing.T) {
	yamlContent := `pipelineName: pipelinetest
pipelineConfig:
  pipelineSource:
    ...
    - processName: data_stream
      plugin:
        ...
        properties:
          ...
          groupId: platform-audit-logs-search-documents
    ...
    - processName: sink_to_es
      plugin:
        ...
        properties:
          type: ELASTICSEARCH
          intervalMillis: '1000'
          exponentialBackoffDelayMillis: '1000'
        destination:
          index: abc
          username: vault://foo?ELASTIC_USERNAME
          password: vault://foo?ELASTIC_PASSWORD
          indexIdCol: id
  ...
  pipelineProperties:
    ...
    additionalProperties:
      flinkTaskCpu: '0.1'
      flinkTaskMemory: 1024m
      flinkTaskSlotNum: '1'
      flinkJobParallelism: '1'
      checkpointInterval: '300000'
      flinkTaskmanagerMemoryManagedFraction: '0.1'
      flinkImage: a.b.c/a/b
`
	doc, err := Parse([]byte(yamlContent))
	require.NoError(t, err)
...
// TestYamleditAddDuplicateKeyBug reproduces the bug where a single "add" operation
// results in the key appearing twice in the output
func TestYamleditAddDuplicateKeyBug(t *testing.T) {
	yamlContent := `pipelineName: test-pipeline
pipelineConfig:
  pipelineProcess:
    - processName: sink_to_es
...
`
	...
	// Apply multiple patches including one "add" operation
	patch := `[
		{"op":"replace","path":"/pipelineName","value":"new-pipeline"},
		{"op":"add","path":"/pipelineConfig/pipelineProcess/0/plugin/destination/host","value":"a.b.c"}
	]`
	...
	if hostCount != 1 {
		t.Errorf("BUG: Expected 'host:' to appear exactly once, but it appears %d times", hostCount)
	}
}

// TestMarshalEmptyFile tests that Marshal works when the original file is empty (new file).
// This reproduces the bug: "surgical edit unsupported; no safe structural rewrite"
// Bug report: When a file doesn't exist in gitops (empty file), adding environment variables
// fails because the surgical edit code cannot handle empty original content.
func TestMarshalEmptyFile(t *testing.T) {
	// Parse empty content (simulating a new file that doesn't exist yet)
	doc, err := Parse([]byte{})
	if err != nil {
		t.Fatalf("Parse empty should succeed, got error: %v", err)
	}

	// Ensure path and set some values (simulating what the accessor does)
	envs := EnsurePath(doc, "service", "envs")
	SetScalarString(envs, "MY_VAR", "my-value")
	SetScalarString(envs, "ANOTHER_VAR", "another-value")

	// Marshal should succeed even though original was empty
	out, err := Marshal(doc)
	if err != nil {
		t.Fatalf("Marshal failed for empty file: %v", err)
	}

	// Verify the output is valid YAML with expected content
	if !strings.Contains(string(out), "MY_VAR: my-value") {
		t.Errorf("Expected MY_VAR in output, got:\n%s", string(out))
	}
	if !strings.Contains(string(out), "ANOTHER_VAR: another-value") {
...
// TestMarshalEmptyFileWithNestedStructure tests adding nested structure to empty file
func TestMarshalEmptyFileWithNestedStructure(t *testing.T) {
	doc, err := Parse([]byte{})
	if err != nil {
		t.Fatalf("Parse empty should succeed, got error: %v", err)
	}
	...
	SetScalarString(service, "name", "my-service")
	SetScalarInt(service, "port", 8080)

	// Marshal should succeed
	out, err := Marshal(doc)
	if err != nil {
		t.Fatalf("Marshal failed for empty file with nested structure: %v", err)
	}

	// Verify content
	if !strings.Contains(string(out), "name: my-service") {
...
// TestMarshalEmptyFileWithEnvVars simulates the exact bug scenario from the issue:
// Adding environment variables to a new file (empty gitops)
func TestMarshalEmptyFileWithEnvVars(t *testing.T) {
	// This simulates what happens when:
	// 1. User creates a new service that doesn't have any YAML file yet
	// 2. User tries to add environment variables via the UI
	// 3. The accessor tries to PatchData on an empty file
	...
	// Simulate what BaseRecordsAccessor.PatchData does for envvars
	// It ensures the path exists and sets scalar values
	envs := EnsurePath(doc, "service", "envs")
	SetScalarString(envs, "KEY", "VALUE")

	// Marshal - this was failing with:
	// "yamledit: surgical edit unsupported; no safe structural rewrite"
	out, err := Marshal(doc)
	if err != nil {
	...
	if !strings.Contains(string(out), expected) {
		t.Errorf("Expected env var key in output, got:\n%s", string(out))
	}
	...
	javaService, ok := data["service"].(map[string]interface{})
	if !ok {
		t.Fatalf("Expected service map, got: %T", data["service"])
	}
	envsMap, ok := javaService["envs"].(map[string]interface{})
	if !ok {
		t.Fatalf("Expected envs map, got: %T", javaService["envs"])
	}
	if envsMap["KEY"] != "VALUE" {
		t.Errorf("Expected env var value not found, got: %v", envsMap["KEY"])
	}
}

// TestMarshalExistingFileStillWorks ensures the fix doesn't break normal file editing
func TestMarshalExistingFileStillWorks(t *testing.T) {
...
`)
	...
	// Add a new env var
	envs := EnsurePath(doc, "service", "envs")
	SetScalarString(envs, "NEW_VAR", "new-value")

	// Marshal should succeed
	out, err := Marshal(doc)
	if err != nil {
		t.Fatalf("Marshal failed for existing file: %v", err)
	}

	// Verify both old and new vars exist
	if !strings.Contains(string(out), "EXISTING_VAR: existing-value") {
		t.Errorf("Expected EXISTING_VAR to be preserved, got:\n%s", string(out))
	}
	// Check for NEW_VAR (may be quoted)
	if !strings.Contains(string(out), "NEW_VAR:") {
		t.Errorf("Expected NEW_VAR in output, got:\n%s", string(out))
	}

	// Verify it's valid YAML and has correct values
	var data map[string]interface{}
	if err := yaml.Unmarshal(out, &data); err != nil {
		t.Fatalf("Output is not valid YAML: %v\nOutput:\n%s", err, string(out))
	}

	javaService := data["service"].(map[string]interface{})
	envsMap := javaService["envs"].(map[string]interface{})
	if envsMap["EXISTING_VAR"] != "existing-value" {
		t.Errorf("EXISTING_VAR value mismatch")
	}
	if envsMap["NEW_VAR"] != "new-value" {
		t.Errorf("NEW_VAR value mismatch, got: %v", envsMap["NEW_VAR"])
	}
}
